...
Projects Unrestricted Hartree-Fock (UHF) molecular orbitals onto a minimal 
basis set (STO-3G) for chemically intuitive population analysis.
...
where S is the minimal basis overlap, S̄ is the cross-overlap <min|ext>, 
and M = Cᵀ·S̄ᵀ·S⁻¹·S̄·C ensures orthonormality.

For UHF, alpha and beta orbitals are independent and must be projected separately.
...
Python API:
    ...
    results = minpop_uhf(mf)  # From existing PySCF calculation
...
def _print_results(results, mol, n_alpha, n_beta):
...
def minpop_uhf(mf, verbose=True):
    ...
    Notes
    -----
    UHF alpha and beta orbitals are independent (different spatial functions),
    so they must be projected separately to the minimal basis.
    """
    mol = mf.mol
    ...
    # UHF has separate alpha and beta MO coefficients and occupations
    ...
    # Key: Project alpha and beta SEPARATELY (they have different spatial functions)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MinPop population analysis for UHF wavefunctions",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python minpop_uhf.py -xyz molecule.xyz -charge 0 -mult 3
    python minpop_uhf.py -xyz molecule.xyz -charge 0 -mult 3 -basis cc-pVDZ
