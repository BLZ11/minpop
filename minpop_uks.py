#!/usr/bin/env python3
"""
Minimum Population (MinPop) Localization Analysis for UKS (KS-DFT) Wavefunctions.
Identical to minpop_uhf.py except the SCF is unrestricted Kohn-Sham (UKS)
with a user-selectable functional (-xc, default pbe).
...
__all__ = ["minpop_uks", "run_uks_from_xyz"]
...
def _print_results(results, mol_min, n_alpha, n_beta):
    ...
    print("=" * 60)
    print("MinPop Analysis (UKS KS-DFT)")
    print("=" * 60)
...
def minpop_uks(mf, verbose=True, azimuthal_gauge=True,
...
def run_uks_from_xyz(xyz_file, charge=0, multiplicity=1, basis='6-31+G',
    ...
    -------
    results : dict
        MinPop analysis results (see minpop_uks)
    
    Examples
    --------
    >>> results = run_uks_from_xyz("ch2.xyz", charge=0, multiplicity=3)
    >>> print(f"Carbon spin: {results['spin_populations'][0]:.4f}")
    """
    ...
    if verbose:
        print(f"KS-DFT: UKS xc={xc} grid level={mf.grids.level}")
    ...
    return minpop_uks(mf, verbose=verbose, azimuthal_gauge=azimuthal_gauge,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MinPop analysis for UKS (KS-DFT) wavefunctions",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python minpop_uks.py -xyz ch2.xyz -charge 0 -mult 3
  python minpop_uks.py -xyz radical.xyz -mult 2 -basis cc-pVDZ
  python minpop_uks.py -xyz snh4.xyz -basis def2-TZVPP     # Auto-detects ECP
  python minpop_uks.py -xyz ch2.xyz -mult 3 -basis cbsb7   # Gaussian CBSB7 (H-Ar)
  python minpop_uks.py -xyz ch2.xyz -mult 3 -basis cbsb3 -basis-dir ./basis
  python minpop_uks.py -xyz singlet_diradical.xyz -mult 1 -guessmix -stable  # broken-symmetry
